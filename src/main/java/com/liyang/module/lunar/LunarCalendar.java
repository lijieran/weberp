...
{
    static final long serialVersionUID = -8125100834729963328L;
    ...
    final private static String[] fullFtv1 = {
        "0101 元旦",
        "0202 湿地日",
        "0210 气象节",
        "0214 情人节",
    ...
    final private static String dayNames1[] = {"周日", "周一", "周二", "周三", "周四", "周五", "周六"};
    ...
    {
        if (get(Calendar.YEAR) < 1901 || get(Calendar.YEAR) > 2100)
        {
            return 1;
        }
     ...
     * @param d
     */
    public void setGregorian(int y, int m, int d) throws ParseException
    {
        setTime(StringUtil.toDate(y + "-" + m + "-" + d));
        chineseYear = 0;
        chineseMonth = 0;
        chineseDate = 0;
        sectionalTerm = 0;
     ...
     * @param date
     */
    public void setGregorian(String date) throws ParseException
    {
        setTime(StringUtil.toDate(date));
        chineseYear = 0;
        chineseMonth = 0;
        chineseDate = 0;
        sectionalTerm = 0;
    ...
    public boolean isWeekFeast(String mmwwdd)
    {
        ...
        if (day == 7) day = 0;
        if (getMonth() != mm) return false;
        if (getWeek() != day) return false;
    ...
    public String getChineseDateText()
    {
        ...
        buf.append(getCyclicalYear() + getCyclicalMonth() + "月 " + getCyclicalDate() + "日\r\n"); //农 属象
    ...
    public String getChineseDateTextSimple()
    {
        //13 号星期5
        StringBuilder buf = new StringBuilder();
        buf.append("今天是");
        buf.append(DateUtil.toString(getTime(),"yyyy年MM月dd日") + " "); //日期
        buf.append(dayNames[getWeek()] + " ");  //星期
        buf.append(getCyclicalYear() + ""); //农历年
        buf.append(getChinaMonthNum(chineseMonth) + "月" + getChinaDayNum(chineseDate) + " "); //农历
...
//        }
    ...
    //周三 2012/3/12 农历三月十一 妇女节
    public String getChineseDateTextSimple1()
    {
    	
        boolean b=false;
        StringBuilder buf = new StringBuilder();
        buf.append(dayNames1[getWeek()] + " ");  //星期
        buf.append(DateUtil.toString(getTime(),"yyyy/M/d") + " "); //日期
       //System.out.println(chineseMonth);
        buf.append("农历"+getChinaMonthNum(chineseMonth) + "月" + getChinaDayNum(chineseDate) + " "); //农历
    ...
    public String[] getChineseDateTextSimple1Arr()
    {
    	String[] sa = new String[4];
    	
        boolean b=false;
        sa[0]=dayNames1[getWeek()];
        sa[1]=DateUtil.toString(getTime(),"yyyy/M/d");
        sa[2]="农历"+getChinaMonthNum(chineseMonth) + "月" + getChinaDayNum(chineseDate);
...
//  三月十一
    public String getChineseDateText1()
    {
...
//  三月十一
    public int getChineseDateText1NoMonth()
    {
    ...
    //农历三月十一 妇女节
    public String getChineseDateTextSimple2()
    {
    ...
    public String[] getChineseDateArray()
    {
    	String[] sa=new String[3];
        
        //buf.append("农历"+getChinaMonthNum(chineseMonth) + "月" + getChinaDayNum(chineseDate) + " "); //农历
        sa[0]="农历"+getChinaMonthNum(chineseMonth) + "月" + getChinaDayNum(chineseDate);
        sa[1]=getCyclicalYear()+"" + getCyclicalMonth() + "月" + getCyclicalDate() + "日";
    ...
    //距小寒还有3天
    public String[] getSolarTermsText(int ibday){
    	String[] sa = new String[3];
    	sa[0]=""; sa[1]=""; sa[2]=""; 
    	
    	int iyear=getYear();
    	int imonth=getMonth(); 
    	int iday=get(Calendar.DATE);
    	int imonthCountD=daysInGregorianMonth(iyear, imonth); //当前月有多少天
    	if (iday>sectionalTerm && iday>principleTerm){ //得取下个月
    		sa[0]="nextMonth"; sa[1]=""+(imonthCountD-iday+1);
    	}else{
    		if (sectionalTerm>principleTerm){
    			if (iday<principleTerm){
    				sa[0]="距"; sa[1]=""+principleTermNames[getMonth() - 1]; sa[2]="还有"+(principleTerm-iday+ibday)+"天";
    			}else if (iday==principleTerm){
    				sa[0]=""; sa[1]=""+principleTermNames[getMonth() - 1]; sa[2]="";
    			}else if (iday==sectionalTerm){
    				sa[0]=""; sa[1]=""+sectionalTermNames[getMonth() - 1]; sa[2]="";
    			}else{
    				sa[0]="距"; sa[1]=""+sectionalTermNames[getMonth() - 1]; sa[2]="还有"+(sectionalTerm-iday+ibday)+"天";
    			}
    		}else{
    			if (iday<sectionalTerm){
    				sa[0]="距"; sa[1]=""+sectionalTermNames[getMonth() - 1]; sa[2]="还有"+(sectionalTerm-iday+ibday)+"天";
    			}else if (iday==principleTerm){
    				sa[0]=""; sa[1]=""+principleTermNames[getMonth() - 1]; sa[2]="";
    			}else if (iday==sectionalTerm){
    	...
    	return sa;
    	//return sectionalTerm+","+principleTerm+","+get(Calendar.DATE)+","+principleTermNames[getMonth() - 1]+","+sectionalTermNames[getMonth() - 1];
    	
//    	if (i1<0 && i2>i1){
//    		if (i2<0) re_s="距"+principleTermNames[getMonth()]+"还有"+i2+"天";
//    		else re_s="距"+principleTermNames[getMonth() - 1]+"还有"+i2+"天";
//    	}else if (i1==0){
//    		re_s="今天为"+sectionalTermNames[getMonth() - 1];
//    	}else if (i2==0){
//    		re_s="今天为"+principleTermNames[getMonth() - 1];
//    	}else{
//    		re_s="距"+sectionalTermNames[getMonth() - 1]+"还有"+i1+"天";
//    	}
    	
//    	if (get(Calendar.DATE) == sectionalTerm)
//        {
//        	buf.append(sectionalTermNames[getMonth() - 1]).append(" ");
//        } else if (get(Calendar.DATE) == principleTerm)
//        {
//        	buf.append(principleTermNames[getMonth() - 1]).append(" ");
//        }
    ...
    public String getCyclicalYear()
    {
        ...
        if (animalIndex < 0) animalIndex = -animalIndex;
        if (getMonth() < 2)
        { //这里应该改为春分后，再分第二年，而不是第二个月后
            return getCyclical(getYear() - 1900 + 36 - 1) + "(" + animalNames[animalIndex-1]+")年";
        }
        return getCyclical(getYear() - 1900 + 36) + "(" + animalNames[animalIndex]+")年";
    }
    ...
    private String getChinaMonthNum(int inum)
    {
    	
    	 return chineseMonthNames[Math.abs(inum) - 1];
    	
    }
    
    private String getChinaDayNum(int inum)
    {
    	...
    	if (inum==30){
    		...
    		return "三十"+monthNames[inum-30-1];
    	}else if (inum>20){
    		return "廿"+monthNames[inum-20-1];
    	}else if (inum>10){
    		return "十"+monthNames[inum-10-1];
    	}else{
    		return "初"+monthNames[inum-1];
    	}
