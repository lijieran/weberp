...
final public class NumberUtil
{
    ...
    //中文金额单位数组
    final private static String[] straChineseUnit = new String[]{"分", "角", "圆", "拾", "佰", "仟", "万", "拾", "佰", "仟", "亿", "拾", "佰", "仟","兆"};
    //中文数字字符数组
    final private static String[] chineseNumber = new String[]{"零", "壹", "贰", "叁", "肆", "伍", "陆", "柒", "捌", "玖", "点"};
    //单位数组
    final private static String[] units = new String[]{"", "", "点", "十", "百", "千", "万", "十", "百", "千", "亿", "十", "百", "千","兆"};
    // 中文大写数字数组
    ...
    public static String toChineseCurrency(BigDecimal bigdMoneyNumber)
    {
        ...
        try
        {
            ...
            while (doubMoneyNumber > 0)
            {
                ...
                {
                    strChineseCurrency = chineseNumber[(int) doubMoneyNumber % 10] + straChineseUnit[ChineseUnitIndex] + strChineseCurrency;
                    bZero = false;
                }
                //零数位的处理
                else
    ...
    {
        if (v2 == 0) return new BigDecimal("0");
        if (scale < 0)
        {
    ...
    public static String toChineseNumber(BigDecimal bigdMoneyNumber, int type)
    {
        ...
        try
        {
            if (bigdMoneyNumber.intValue() == 0) return "零";
            //处理小数部分，四舍五入
            double doubMoneyNumber = Math.round(bigdMoneyNumber.doubleValue() * 100);
            //是否负数
            boolean bNegative = doubMoneyNumber < 0;
            ...
            while (doubMoneyNumber > 0)
            {
                ...
                if (doubMoneyNumber % 10 > 0)
                {
                    strChineseCurrency = numeric[(int) doubMoneyNumber % 10] + units[ChineseUnitIndex] + strChineseCurrency;
                    bZero = false;
                }
                //零数位的处理
                else
        ...
        if (strChineseCurrency.endsWith(units[2])) strChineseCurrency = strChineseCurrency.substring(0,strChineseCurrency.length() -1);
        return strChineseCurrency;
    }
